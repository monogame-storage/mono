-- THE DARK ROOM: LETHE (Sonar Edition)
-- Agent 31 | Wave 4 | Polished from Agent 29 + Agent 27 sonar
-- 160x120 | 2-bit (4 shades: 0-3) | mode(2)
-- D-Pad: Move cursor/Turn | A: Interact/Ping | B: Inventory | START: Pause
...
local function snd_entity_step(intensity)
  if intensity < 0.05 then return end
  local freq = 200 - intensity * 100
  sfx_wave(2, "square")
  sfx_tone(2, freq, freq * 0.8, 0.02 + intensity * 0.02)
end
...
-- SONAR SYSTEM
----------------------------------------------
local SONAR_CD = 40       -- cooldown frames
local SONAR_RING_SPEED = 3 -- pixels per frame expansion
local SONAR_MAX_R = 100    -- max ring radius
local SONAR_FADE = 90      -- frames objects stay visible after ping

local sonar_cooldown = 0
local sonar_ring_active = false
local sonar_ring_radius = 0
local sonar_reveal = {}    -- {name=string, timer=int} revealed hotspots

local function sonar_is_revealed(name)
  if has_light then return true end
  local r = sonar_reveal[name]
  return r and r > 0
end

local function sonar_reveal_hotspot(name)
  sonar_reveal[name] = SONAR_FADE
end

----------------------------------------------
-- ENTITY SYSTEM (stalker)
----------------------------------------------
local entity = {
  room = 3,           -- starts in storage
  active = false,     -- activates after first ping
  alert = false,      -- alerted by sonar
  alert_room = 1,     -- room where ping was heard
  timer = 0,
  chase_timer = 0,    -- when >0, entity is approaching player room
  in_room = false,    -- entity is currently in player's room
  scare_flash = 0,
  shake_timer = 0,
  growl_cd = 0,
}

local function entity_room_adjacent(r1, r2)
  -- rooms connected by doors
  local adj = {
    [1] = {2}, [2] = {1, 3, 4, 5}, [3] = {2},
    [4] = {2}, [5] = {2, 6}, [6] = {5, 7}, [7] = {6},
  }
  if not adj[r1] then return false end
  for _, v in ipairs(adj[r1]) do
    if v == r2 then return true end
  end
  return false
end
...
----------------------------------------------
-- SONAR-AWARE DRAWING WRAPPER
-- Objects only draw if revealed by sonar or has_light
----------------------------------------------
local function sonar_draw_wrap(name, draw_fn)
  return function(s, hs)
    if sonar_is_revealed(name) then
      draw_fn(s, hs)
    else
      -- Draw dim outline hint (barely visible shape)
      local c = DARK
      if fcount % 30 < 4 then c = BLACK end
      -- Just a faint rectangle outline for the hotspot area
      -- This gives a subtle hint something is there
    end
  end
end
...
local function build_hotspots()
  ...
  hotspots[11] = {
    {x=60, y=42, w=28, h=36, name="cell_door",
      draw=function(s, hs)
        if not sonar_is_revealed("cell_door") then return end
      ...
      interact=function()
        if not sonar_is_revealed("cell_door") then
          show_msg("Darkness. Try pinging.", 40)
          return
        end
  ...
  hotspots[12] = {
    {x=40, y=44, w=50, h=28, name="cell_wall",
      draw=function(s, hs)
        if not sonar_is_revealed("cell_wall") then return end
      ...
      interact=function()
        if not sonar_is_revealed("cell_wall") then
          show_msg("You feel rough wall.", 40)
          return
        end
        if has_light then
  ...
  hotspots[13] = {
    {x=45, y=50, w=50, h=20, name="cell_cot",
      draw=function(s, hs)
        if not sonar_is_revealed("cell_cot") then return end
        local c = has_light and WHITE or LIGHT
        local bg = has_light and LIGHT or DARK
      ...
      interact=function()
        if not sonar_is_revealed("cell_cot") then
          show_msg("Something soft. A cot?", 40)
          return
        end
        if not has_item("journal1") and has_light then
          add_item("journal1")
  ...
  hotspots[14] = {
    {x=50, y=52, w=36, h=20, name="cell_floor",
      draw=function(s, hs)
        if not sonar_is_revealed("cell_floor") then return end
        local c = has_light and LIGHT or DARK
      ...
      interact=function()
        if not sonar_is_revealed("cell_floor") then
          show_msg("Your foot hits something.", 40)
          return
        end
        if not has_item("matches") then
          add_item("matches")
  ...
  hotspots[21] = {
    {x=55, y=42, w=28, h=36, name="corr_lab_door",
      draw=function(s, hs)
        if not sonar_is_revealed("corr_lab_door") then return end
      ...
      interact=function()
        if not sonar_is_revealed("corr_lab_door") then
          show_msg("A wall? No... a door.", 40)
          return
        end
  ...
  hotspots[22] = {
    {x=30, y=44, w=34, h=24, name="corr_notices",
      draw=function(s, hs)
        if not sonar_is_revealed("corr_notices") then return end
        local c = has_light and WHITE or LIGHT
        rect(s, hs.x, hs.y, hs.w, hs.h, c)
      ...
      interact=function()
        if not sonar_is_revealed("corr_notices") then
          show_msg("Paper rustles against your hand.", 40)
          return
        end
        if has_light then
    ...
    {x=95, y=44, w=26, h=34, name="corr_office_door",
      draw=function(s, hs)
        if not sonar_is_revealed("corr_office_door") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, false)
        if has_light then text(s, "OFC", hs.x + 4, hs.y + 14, DARK) end
      end,
      interact=function()
        if not sonar_is_revealed("corr_office_door") then
          show_msg("Something here...", 40)
          return
        end
        if has_item("keycard") or held_item == "keycard" then
          snd_door()
  ...
  hotspots[23] = {
    {x=60, y=42, w=28, h=36, name="corr_cell_door",
      draw=function(s, hs)
        if not sonar_is_revealed("corr_cell_door") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, true)
        if has_light then text(s, "CELL", hs.x + 4, hs.y + 14, LIGHT) end
      end,
      interact=function()
        if not sonar_is_revealed("corr_cell_door") then
          show_msg("An opening. Familiar.", 40)
          return
        end
        snd_door()
        trans_timer = 15
  ...
  hotspots[24] = {
    {x=28, y=42, w=30, h=14, name="corr_pipes",
      draw=function(s, hs)
        if not sonar_is_revealed("corr_pipes") then return end
        local c = has_light and LIGHT or DARK
        for i = 0, 2 do
      ...
      interact=function()
        if not sonar_is_revealed("corr_pipes") then
          show_msg("Metal pipes. Dripping.", 40)
          return
        end
        if not flags.keycard_found and has_light then
    ...
    {x=85, y=48, w=26, h=30, name="corr_storage_door",
      draw=function(s, hs)
        if not sonar_is_revealed("corr_storage_door") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, false)
        if has_light then text(s, "STOR", hs.x + 2, hs.y + 10, DARK) end
      end,
      interact=function()
        if not sonar_is_revealed("corr_storage_door") then
          show_msg("A door, maybe?", 40)
          return
        end
        snd_door()
  ...
  hotspots[31] = {
    {x=35, y=42, w=30, h=36, name="stor_shelf_l",
      draw=function(s, hs)
        if not sonar_is_revealed("stor_shelf_l") then return end
      ...
      interact=function()
        if not sonar_is_revealed("stor_shelf_l") then
          show_msg("Shelves in the dark.", 40)
          return
        end
        if not has_item("wire") and has_light then
          add_item("wire")
    ...
    {x=80, y=42, w=30, h=36, name="stor_shelf_r",
      draw=function(s, hs)
        if not sonar_is_revealed("stor_shelf_r") then return end
        draw_bookshelf(s, hs.x, hs.y)
      ...
      interact=function()
        if not sonar_is_revealed("stor_shelf_r") then
          show_msg("More shelves.", 40)
          return
        end
        if not has_item("metal_rod") and has_light then
          add_item("metal_rod")
  ...
  hotspots[32] = {
    {x=40, y=48, w=24, h=18, name="stor_box1",
      draw=function(s, hs)
        if not sonar_is_revealed("stor_box1") then return end
        draw_box(s, hs.x, hs.y, hs.w, hs.h, false)
        if has_light then text(s, "FUSE", hs.x + 4, hs.y + 6, DARK) end
      end,
      interact=function()
        if not sonar_is_revealed("stor_box1") then
    ...
    {x=80, y=48, w=24, h=18, name="stor_box2",
      draw=function(s, hs)
        if not sonar_is_revealed("stor_box2") then return end
        draw_box(s, hs.x, hs.y, hs.w, hs.h, false)
        if has_light then text(s, "TAPE", hs.x + 4, hs.y + 6, DARK) end
      end,
      interact=function()
        if not sonar_is_revealed("stor_box2") then
          show_msg("Something in a box.", 40)
          return
        end
  ...
  hotspots[33] = {
    {x=60, y=42, w=28, h=36, name="stor_door",
      draw=function(s, hs)
        if not sonar_is_revealed("stor_door") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, true)
        if has_light then text(s, "CORR", hs.x + 2, hs.y + 14, LIGHT) end
      end,
      interact=function()
        if not sonar_is_revealed("stor_door") then
          show_msg("A way out?", 40)
          return
        end
        snd_door()
        trans_timer = 15
  ...
  hotspots[34] = {
    {x=40, y=44, w=50, h=28, name="stor_chem",
      draw=function(s, hs)
        if not sonar_is_revealed("stor_chem") then return end
        local c = has_light and WHITE or LIGHT
        rect(s, hs.x, hs.y, hs.w, hs.h, c)
        if has_light then
          for i = 0, 4 do
      ...
      interact=function()
        if not sonar_is_revealed("stor_chem") then
          show_msg("Chemical smell. Strong.", 40)
          return
        end
        if not has_item("journal2") and has_light then
          add_item("journal2")
  ...
  hotspots[41] = {
    {x=60, y=46, w=18, h=14, name="lab_fusebox",
      draw=function(s, hs)
        if not sonar_is_revealed("lab_fusebox") then return end
        draw_fuse_box(s, hs.x, hs.y, flags.lab_powered)
      end,
      interact=function()
        if not sonar_is_revealed("lab_fusebox") then
          show_msg("Something on the wall.", 40)
          return
        end
    ...
    {x=90, y=42, w=28, h=22, name="lab_terminal",
      draw=function(s, hs)
        if not sonar_is_revealed("lab_terminal") then return end
        draw_terminal(s, hs.x, hs.y, flags.lab_powered)
      end,
      interact=function()
        if not sonar_is_revealed("lab_terminal") then
          show_msg("Electronics. Dead.", 40)
          return
        end
  ...
  hotspots[42] = {
    {x=45, y=46, w=40, h=30, name="lab_chair",
      draw=function(s, hs)
        if not sonar_is_revealed("lab_chair") then return end
        draw_chair(s, hs.x, hs.y)
      end,
      interact=function()
        if not sonar_is_revealed("lab_chair") then
          show_msg("Bulky. Straps. Cold metal.", 40)
          return
        end
        if has_light then
  ...
  hotspots[43] = {
    {x=60, y=42, w=28, h=36, name="lab_door",
      draw=function(s, hs)
        if not sonar_is_revealed("lab_door") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, true)
        if has_light then text(s, "CORR", hs.x + 2, hs.y + 14, LIGHT) end
      end,
      interact=function()
        if not sonar_is_revealed("lab_door") then
          show_msg("A door?", 40)
          return
        end
        snd_door()
  ...
  hotspots[44] = {
    {x=40, y=52, w=60, h=16, name="lab_desk",
      draw=function(s, hs)
        if not sonar_is_revealed("lab_desk") then return end
      ...
      interact=function()
        if not sonar_is_revealed("lab_desk") then
          show_msg("Furniture. A desk.", 40)
          return
        end
  ...
  hotspots[51] = {
    {x=50, y=44, w=40, h=28, name="ofc_painting",
      draw=function(s, hs)
        if not sonar_is_revealed("ofc_painting") then return end
      ...
      interact=function()
        if not sonar_is_revealed("ofc_painting") then
          show_msg("A frame on the wall.", 40)
          return
        end
  ...
  hotspots[52] = {
    {x=55, y=44, w=22, h=30, name="ofc_cabinet",
      draw=function(s, hs)
        if not sonar_is_revealed("ofc_cabinet") then return end
        draw_filing_cabinet(s, hs.x, hs.y, flags.evidence_found)
      end,
      interact=function()
        if not sonar_is_revealed("ofc_cabinet") then
          show_msg("Metal. Drawers?", 40)
          return
        end
    ...
    {x=90, y=50, w=24, h=22, name="ofc_safe",
      draw=function(s, hs)
        if not sonar_is_revealed("ofc_safe") then return end
      ...
      interact=function()
        if not sonar_is_revealed("ofc_safe") then
          show_msg("Heavy metal. A safe?", 40)
          return
        end
  ...
  hotspots[53] = {
    {x=40, y=42, w=28, h=36, name="ofc_door_corr",
      draw=function(s, hs)
        if not sonar_is_revealed("ofc_door_corr") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, true)
        if has_light then text(s, "CORR", hs.x + 2, hs.y + 14, LIGHT) end
      end,
      interact=function()
        if not sonar_is_revealed("ofc_door_corr") then
          show_msg("Doorway.", 40)
          return
        end
        snd_door()
    ...
    {x=100, y=48, w=24, h=30, name="ofc_door_exit",
      draw=function(s, hs)
        if not sonar_is_revealed("ofc_door_exit") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, false)
        if has_light then text(s, "EXIT", hs.x + 2, hs.y + 10, DARK) end
      end,
      interact=function()
        if not sonar_is_revealed("ofc_door_exit") then
          show_msg("Something here...", 40)
          return
        end
  ...
  hotspots[54] = {
    {x=40, y=52, w=60, h=16, name="ofc_desk",
      draw=function(s, hs)
        if not sonar_is_revealed("ofc_desk") then return end
        draw_desk(s, hs.x, hs.y)
        if has_light then
          for i = 0, 3 do
      ...
      interact=function()
        if not sonar_is_revealed("ofc_desk") then
          show_msg("Papers rustle.", 40)
          return
        end
  ...
  hotspots[61] = {
    {x=50, y=40, w=40, h=38, name="exit_blast_door",
      draw=function(s, hs)
        if not sonar_is_revealed("exit_blast_door") then return end
        local c = has_light and WHITE or LIGHT
        rect(s, hs.x, hs.y, hs.w, hs.h, c)
      ...
      interact=function()
        if not sonar_is_revealed("exit_blast_door") then
          show_msg("Massive door. Cold steel.", 40)
          return
        end
        if flags.escaped then
          state = "ending"
    ...
    {x=96, y=46, w=16, h=20, name="exit_keypad",
      draw=function(s, hs)
        if not sonar_is_revealed("exit_keypad") then return end
        draw_keypad(s, hs.x, hs.y)
      end,
      interact=function()
        if not sonar_is_revealed("exit_keypad") then
          show_msg("Buttons. A keypad?", 40)
          return
        end
  ...
  hotspots[62] = {
    {x=50, y=50, w=30, h=20, name="exit_debris",
      draw=function(s, hs)
        if not sonar_is_revealed("exit_debris") then return end
        local c = has_light and LIGHT or DARK
        for i = 0, 3 do
      ...
      interact=function()
        if not sonar_is_revealed("exit_debris") then
          show_msg("Rubble underfoot.", 40)
          return
        end
        if not has_item("crowbar") and has_light then
          add_item("crowbar")
  ...
  hotspots[63] = {
    {x=60, y=42, w=28, h=36, name="exit_back",
      draw=function(s, hs)
        if not sonar_is_revealed("exit_back") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, true)
        if has_light then text(s, "OFC", hs.x + 4, hs.y + 14, LIGHT) end
      end,
      interact=function()
        if not sonar_is_revealed("exit_back") then
          show_msg("A way back.", 40)
          return
        end
        snd_door()
        trans_timer = 15
  ...
  hotspots[64] = {
    {x=55, y=44, w=28, h=34, name="exit_garden_door",
      draw=function(s, hs)
        if not sonar_is_revealed("exit_garden_door") then return end
      ...
      interact=function()
        if not sonar_is_revealed("exit_garden_door") then
          show_msg("Something here. Locked.", 40)
          return
        end
  ...
  hotspots[71] = {
    {x=60, y=48, w=20, h=12, name="garden_recorder",
      draw=function(s, hs)
        if not sonar_is_revealed("garden_recorder") then return end
      ...
      interact=function()
        if not sonar_is_revealed("garden_recorder") then
          show_msg("A machine. Small.", 40)
          return
        end
  ...
  hotspots[72] = {
    {x=40, y=44, w=50, h=28, name="garden_plants",
      draw=function(s, hs)
        if not sonar_is_revealed("garden_plants") then return end
        local c = has_light and LIGHT or DARK
        for i = 0, 4 do
      ...
      interact=function()
        if not sonar_is_revealed("garden_plants") then
          show_msg("Something growing here.", 40)
          return
        end
  ...
  hotspots[73] = {
    {x=60, y=42, w=28, h=36, name="garden_door",
      draw=function(s, hs)
        if not sonar_is_revealed("garden_door") then return end
        draw_door(s, hs.x, hs.y, hs.w, hs.h, true)
        if has_light then text(s, "EXIT", hs.x + 2, hs.y + 14, LIGHT) end
      end,
      interact=function()
        if not sonar_is_revealed("garden_door") then
          show_msg("The way back.", 40)
          return
        end
        snd_door()
        trans_timer = 15
  ...
  hotspots[74] = {
    {x=45, y=42, w=40, h=30, name="garden_window",
      draw=function(s, hs)
        if not sonar_is_revealed("garden_window") then return end
      ...
      interact=function()
        if not sonar_is_revealed("garden_window") then
          show_msg("Air. Fresh air.", 40)
          return
        end
...
local function do_sonar_ping()
  ...
  sonar_ring_active = true
  sonar_ring_radius = 0
  snd_ping()

  -- Reveal all hotspots on current wall
  local spots = get_wall_hotspots(cur_room, cur_dir)
  for _, hs in ipairs(spots) do
    if hs.name then
      sonar_reveal_hotspot(hs.name)
    end
  end

  -- Also reveal adjacent walls' hotspots (sound echoes)
  local left_dir = ((cur_dir - 2) % 4) + 1
  local right_dir = (cur_dir % 4) + 1
  local lspots = get_wall_hotspots(cur_room, left_dir)
  for _, hs in ipairs(lspots) do
    if hs.name then sonar_reveal_hotspot(hs.name) end
  end
  local rspots = get_wall_hotspots(cur_room, right_dir)
  for _, hs in ipairs(rspots) do
    if hs.name then sonar_reveal_hotspot(hs.name) end
  end
  ...
  if entity.active then
    entity.alert = true
    entity.alert_room = cur_room
    snd_alert()
    show_msg("...something heard that.", 50)
  end

  -- Activate entity after first ping
...
local function update_sonar()
  ...
  -- Expand ring
  if sonar_ring_active then
    sonar_ring_radius = sonar_ring_radius + SONAR_RING_SPEED
    if sonar_ring_radius > SONAR_MAX_R then
      sonar_ring_active = false
    end
  end

  -- Decay reveal timers
  for name, t in pairs(sonar_reveal) do
    if t > 0 then
      sonar_reveal[name] = t - 1
    end
  end
end
...
local function update_entity()
  ...
  -- Entity growl cooldown
  if entity.growl_cd > 0 then entity.growl_cd = entity.growl_cd - 1 end
  ...
  if entity.alert then
    -- If alerted, move toward player's room
    if not same_room and entity_room_adjacent(entity.room, entity.alert_room) then
      entity.chase_timer = entity.chase_timer + 1
      -- After some frames, entity enters the room
      if entity.chase_timer > 60 then
        entity.room = entity.alert_room
        entity.chase_timer = 0
        entity.in_room = true
        snd_entity_growl()
        show_msg("...it enters the room...", 60)
      elseif entity.chase_timer == 20 then
        -- Footsteps approaching
        snd_entity_step(0.5)
        show_msg("...footsteps...", 30)
      end
    elseif same_room then
      entity.in_room = true
      -- In the same room -- approaching
      entity.chase_timer = entity.chase_timer + 1
      if entity.chase_timer > 90 then
        -- CAUGHT! Jumpscare
        snd_jumpscare()
        entity.scare_flash = 12
        entity.shake_timer = 15
        entity.chase_timer = 0
        entity.alert = false
        entity.in_room = false
        -- Death: reset player position
        show_typewriter("Darkness. A grip like iron on your throat. The thing that wears Wren's face leans close. Its breath is cold and smells of formaldehyde. 'You forgot again,' it whispers. 'Good.' You feel the needle. You feel your memories dissolving like sugar in water. And then nothing.\n\n...You wake in darkness. No memory. No name.", 1)
        -- Reset entity to another room
        entity.room = 3
        cur_room = 1
        cur_dir = DIR_N
        cur_x = 80
        ...
        heartbeat_active = true
        heartbeat_timer = 60
      elseif entity.chase_timer % 15 == 0 then
        snd_entity_step(0.8)
        if entity.growl_cd <= 0 and entity.chase_timer > 40 then
          snd_entity_growl()
          entity.growl_cd = 30
        end
      end
    else
      -- Not adjacent, wander toward player
      entity.timer = entity.timer + 1
      if entity.timer % 90 == 0 then
        -- Move one room closer
        local adj = {
          [1] = {2}, [2] = {1,3,4,5}, [3] = {2},
          [4] = {2}, [5] = {2,6}, [6] = {5,7}, [7] = {6},
        }
        local options = adj[entity.room] or {}
        -- Pick room closest to player
        local best = nil
        for _, r in ipairs(options) do
          if r == entity.alert_room then best = r break end
          if entity_room_adjacent(r, entity.alert_room) then best = r end
        end
        if best then entity.room = best end
      end
    end
  else
    -- Not alerted: random patrol
    entity.in_room = false
    entity.chase_timer = 0
    if entity.timer % 120 == 0 then
      local adj = {
        [1] = {2}, [2] = {1,3,4,5}, [3] = {2},
        [4] = {2}, [5] = {2,6}, [6] = {5,7}, [7] = {6},
      }
      local options = adj[entity.room] or {}
      if #options > 0 then
        entity.room = options[math.random(#options)]
      end
    end
    -- If entity wanders into player's room by chance
    if same_room and math.random(200) < 3 then
      entity.alert = true
      entity.alert_room = cur_room
      show_msg("...it found you...", 50)
      snd_entity_growl()
    end
  end

  -- Heartbeat based on entity proximity
  if same_room or entity_room_adjacent(entity.room, cur_room) then
...
local function update_play_input()
  ...
  if btnp("left") then
    if cur_x <= 25 then
      ...
      cur_x = 80
      snd_turn()
      -- Clear reveal on turn (you're looking at a new wall)
    else
      cur_x = cur_x - 9
    end
  end
  ...
  if btnp("a") then
    ...
    if not hit then
      -- No hotspot hit: emit sonar ping
      do_sonar_ping()
    end
  end
...
----------------------------------------------
-- DRAW SONAR RING EFFECT
----------------------------------------------
local function draw_sonar_ring(s)
  if not sonar_ring_active then return end

  local cx, cy = 80, 55  -- center of first-person view
  local r = sonar_ring_radius
  local fade = 1.0 - (r / SONAR_MAX_R)
  local col = fade > 0.5 and LIGHT or DARK

  -- Draw expanding ring as circle outline
  local segs = 40
  for i = 0, segs - 1 do
    local a = (i / segs) * math.pi * 2
    local px = cx + math.cos(a) * r
    local py = cy + math.sin(a) * r * 0.6  -- perspective squash
    local sx = math.floor(px)
    local sy = math.floor(py)
    if sx >= 0 and sx < W and sy >= 0 and sy < H - 10 then
      pix(s, sx, sy, col)
      -- Thicken inner ring
      if fade > 0.7 then
        if sx + 1 < W then pix(s, sx + 1, sy, col) end
      end
    end
  end

  -- Central pulse at origin
  if r < 10 then
    local pc = WHITE
    circ(s, cx, cy, math.floor(r / 2), pc)
  end
end

----------------------------------------------
-- DRAW ENTITY EYES (horror element)
----------------------------------------------
local function draw_entity_eyes(s)
  if not entity.active then return end
  if not entity.in_room then
    -- If adjacent room, faint eyes at edge
    if entity_room_adjacent(entity.room, cur_room) then
      local blink = math.floor(fcount / 25) % 5
      if blink < 2 then
        local ex = 12 + math.sin(fcount * 0.03) * 6
        local ey = 55 + math.cos(fcount * 0.02) * 4
        pix(s, math.floor(ex) - 1, math.floor(ey), DARK)
        pix(s, math.floor(ex) + 1, math.floor(ey), DARK)
      end
    end
    return
  ...
  -- Entity is in room - eyes watching
  local blink = math.floor(fcount / 20) % 5
  if blink < 3 then
    local ex = 80 + math.sin(fcount * 0.04) * 30
    local ey = 50 + math.cos(fcount * 0.03) * 8
    local intensity = entity.chase_timer / 90.0
    local col = intensity > 0.5 and WHITE or LIGHT
    pix(s, math.floor(ex) - 2, math.floor(ey), col)
    pix(s, math.floor(ex) + 2, math.floor(ey), col)
    -- Closer = bigger eyes
    if intensity > 0.6 then
      pix(s, math.floor(ex) - 2, math.floor(ey) - 1, DARK)
      pix(s, math.floor(ex) + 2, math.floor(ey) - 1, DARK)
    end
  end
end
...
local function draw_game(s)
  ...
  -- Draw hotspots for current wall (sonar-gated)
  local spots = get_wall_hotspots(cur_room, cur_dir)
  for _, hs in ipairs(spots) do
    if hs.draw then hs.draw(s, hs) end
  end

  -- Sonar ring effect overlay
  draw_sonar_ring(s)
  ...
  -- Sonar cooldown bar in HUD
  if sonar_cooldown > 0 then
    local bar_w = math.floor((sonar_cooldown / SONAR_CD) * 24)
    rectf(s, 80, H - 4, bar_w, 2, DARK)
    -- Label
    text(s, "CD", 80, H - 8, DARK)
  else
    if fcount % 60 < 40 then
      text(s, "A:PING", 80, H - 8, DARK)
    end
  end
  ...
  if bits > 0 then
    for i = 1, math.min(bits, 8) do
      pix(s, 110 + i * 2, H - 2, DARK)
    end
  end

  -- Entity proximity warning (heartbeat visual)
  if entity.active and (entity.room == cur_room or entity_room_adjacent(entity.room, cur_room)) then
    local pulse = math.sin(fcount * 0.15)
    if pulse > 0.5 then
      pix(s, W - 6, H - 6, LIGHT)
      pix(s, W - 4, H - 6, LIGHT)
    end
  ...
  if trans_timer > 0 then
    ...
    if trans_timer == 0 and trans_target_room then
      ...
      cur_x = 80
      cur_y = 60
      sonar_reveal = {}  -- Clear reveals on room change
  ...
  -- Static/horror overlay when entity is close
  if entity.in_room and entity.chase_timer > 30 then
    local intensity = (entity.chase_timer - 30) / 60.0
    local count = math.floor(intensity * 40)
    for _ = 1, count do
      local sx = math.random(0, W - 1)
      local sy = math.random(0, H - 11)
...
local title_text = "You wake in darkness. No memory. No name. Only the cold. Somewhere, something breathes."
...
local function draw_title(s)
  ...
  if flicker then
    text(s, "THE DARK ROOM", 80, 14, WHITE, ALIGN_HCENTER)
  end
  text(s, "L E T H E", 80, 26, LIGHT, ALIGN_HCENTER)
  text(s, "sonar edition", 80, 35, DARK, ALIGN_HCENTER)
  ...
  for i, ln in ipairs(tw_lines_title) do
    text(s, ln, 80, 46 + (i - 1) * 8, DARK, ALIGN_HCENTER)
  end
  ...
  text(s, "A:Ping/Act B:Inventory", 80, 100, DARK, ALIGN_HCENTER)
  text(s, "Find. Remember. Escape.", 80, 110, DARK, ALIGN_HCENTER)
...
local function update_demo()
  ...
  if demo_wait >= 60 then
    ...
    snd_turn()
    -- Simulate sonar ping in demo
    sonar_ring_active = true
    sonar_ring_radius = 0
    snd_ping()
    -- Reveal current wall hotspots
    local spots = get_wall_hotspots(cur_room, cur_dir)
    for _, hs in ipairs(spots) do
      if hs.name then sonar_reveal_hotspot(hs.name) end
    end

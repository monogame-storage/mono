-- THE DARK ROOM: LETHE
-- Agent 34 | Wave 4 | Visual Polish
-- Based on Agent 29 (Wave 3) - ALL narrative/gameplay intact
...
local trans_phase = "out" -- "out" = closing, "in" = opening
...
local function init_particles()
  ...
  for i = 1, MAX_PARTICLES do
    particles[i] = {
      ...
      life = math.random(60, 180),
      max_life = 180,
      kind = "dust", -- dust, drip, spark, firefly
    }
  end
end
...
local function update_particles()
  for i = 1, #particles do
    ...
    if p.kind == "drip" then
      p.vy = p.vy + 0.05 -- gravity
    elseif p.kind == "firefly" then
      p.vx = p.vx + (math.random() - 0.5) * 0.1
      p.vy = p.vy + (math.random() - 0.5) * 0.1
      p.vx = p.vx * 0.95
      p.vy = p.vy * 0.95
    end

    -- Reset dead particles
    if p.life <= 0 or p.x < 0 or p.x >= W or p.y < 0 or p.y >= H - 10 then
      p.life = math.random(40, 160)
      p.max_life = p.life
      -- Room-specific particle spawning
      if cur_room == 2 or cur_room == 3 then
        -- Corridor/storage: drips
        if math.random(3) == 1 then
          p.kind = "drip"
          p.x = math.random(20, W - 20)
          p.y = 0
          p.vx = 0
      ...
      elseif cur_room == 4 then
        -- Lab: sparks near ceiling
        if math.random(4) == 1 then
          p.kind = "spark"
          p.x = math.random(40, W - 40)
          p.y = math.random(5, 20)
          p.vx = (math.random() - 0.5) * 1.5
          p.vy = 0.5 + math.random() * 1.0
          p.life = math.random(8, 20)
          p.max_life = p.life
        else
          p.kind = "dust"
          p.x = math.random(W)
      ...
      elseif cur_room == 7 then
        -- Garden: fireflies
        if math.random(2) == 1 then
          p.kind = "firefly"
          p.x = math.random(20, W - 20)
          p.y = math.random(20, H - 20)
          p.vx = (math.random() - 0.5) * 0.4
          p.vy = (math.random() - 0.5) * 0.4
          p.life = math.random(80, 200)
          p.max_life = p.life
        else
          p.kind = "dust"
          p.x = math.random(W)
...
local function draw_particles(s)
  for i = 1, #particles do
    ...
    if px >= 0 and px < W and py >= 0 and py < H - 10 then
      if p.kind == "dust" then
        local c = fade > 0.5 and DARK or (fade > 0.2 and DARK or BLACK)
        if has_light and fade > 0.6 then c = LIGHT end
        pix(s, px, py, c)
      elseif p.kind == "drip" then
        local c = has_light and LIGHT or DARK
        pix(s, px, py, c)
        if py > 0 then pix(s, px, py - 1, DARK) end
      elseif p.kind == "spark" then
        local c = fade > 0.5 and WHITE or LIGHT
        pix(s, px, py, c)
      elseif p.kind == "firefly" then
        -- Fireflies pulse brightness
        local pulse = math.sin(frame() * 0.1 + i * 2.0)
        if pulse > 0.2 then
          pix(s, px, py, WHITE)
...
----------------------------------------------
local function draw_vignette(s)
  -- Darken edges of screen with distance-based falloff
  local cx, cy = W / 2, (H - 10) / 2
  local max_r = math.sqrt(cx * cx + cy * cy)
  -- Strength depends on light
  local strength = has_light and 0.55 or 0.75
  -- Heartbeat pulse
  if heartbeat_active and heartbeat_timer > 0 then
    local pulse = math.sin(heartbeat_timer * 0.4)
    strength = strength + pulse * 0.08
  end
  -- Only process edge bands for performance
  local band = 20
  for y = 0, H - 11 do
    for x = 0, W - 1 do
      -- Only darken pixels near edges
      local ex = math.min(x, W - 1 - x)
      local ey = math.min(y, H - 11 - y)
      local edge_dist = math.min(ex, ey)
      if edge_dist < band then
        local t = 1.0 - (edge_dist / band)
        t = t * t * strength
        if t > 0.6 then
          pix(s, x, y, BLACK)
        elseif t > 0.3 then
          local cur = gpix(s, x, y)
...
----------------------------------------------
-- VISUAL POLISH: ROOM-SPECIFIC DITHERING
----------------------------------------------
...
local function room_dither_test(room, x, y)
  -- Returns true if pixel should be the darker shade in a dither pair
  if room == 1 then
    -- Cell: diagonal hatch (claustrophobic scratches)
    return ((x + y) % 4 == 0) or ((x - y) % 6 == 0)
  elseif room == 2 then
    -- Corridor: horizontal scanline (fluorescent flicker)
    return (y % 3 == 0)
  elseif room == 3 then
    -- Storage: noise stipple (cluttered chaos)
    local hash = (x * 7 + y * 13 + x * y) % 17
    return hash < 5
  elseif room == 4 then
    -- Lab: Bayer ordered dither (clinical precision)
    local bx = (x % 4) + 1
    local by = (y % 4) + 1
    return BAYER4[by][bx] < 8
  elseif room == 5 then
    -- Office: crosshatch (bureaucratic)
    return ((x + y) % 3 == 0) or ((x - y) % 5 == 0)
  elseif room == 6 then
    -- Exit hall: vertical stripes (imposing gate)
    return (x % 4 < 2)
  elseif room == 7 then
    -- Garden: organic wave pattern
    local wave = math.sin(x * 0.15 + y * 0.1) * 3
    return ((y + math.floor(wave)) % 4 == 0)
  end
  return (x + y) % 2 == 0
end

----------------------------------------------
-- VISUAL POLISH: FLICKER EFFECT
----------------------------------------------
local flicker_val = 0
local function update_flicker()
  -- Random brightness variation for lit rooms
  if has_light then
    if math.random(30) == 1 then
      flicker_val = -1 -- brief dimming
    elseif math.random(60) == 1 then
      flicker_val = 1 -- brief brightening
    else
      if flicker_val ~= 0 then
        if math.random(3) == 1 then flicker_val = 0 end
      end
    end
  else
    flicker_val = 0
...
----------------------------------------------
-- ROOM-SPECIFIC WALL TEXTURES
----------------------------------------------
local function draw_room_texture(s, room)
  -- Overlay subtle room-specific texture on the back wall area
  if room == 1 then
    -- Cell: scratch marks on walls
    for i = 0, 12 do
      local sx = 24 + i * 10 + (i % 3) * 2
      local sy = 42 + (i % 4) * 3
      local c = has_light and DARK or BLACK
      if sx < W - 24 then
        pix(s, sx, sy, c)
        pix(s, sx + 1, sy + 1, c)
        pix(s, sx + 2, sy + 2, c)
  ...
  elseif room == 2 then
    -- Corridor: emergency light pools on floor
    local pool_c = has_light and LIGHT or DARK
    for i = 0, 2 do
      local px = 30 + i * 45
      local py = 82
      for dy = 0, 3 do
        for dx = -3 + dy, 3 - dy do
          local xx = px + dx
          if xx >= 20 and xx < W - 20 then
            pix(s, xx, py + dy, pool_c)
          end
        end
      end
    end
  ...
  elseif room == 3 then
    -- Storage: scattered floor debris
    local seed = 42
    for i = 0, 8 do
      local dx = (seed * (i + 1) * 7) % (W - 44) + 22
      local dy = 82 + ((seed * (i + 3) * 11) % 6)
      local c = has_light and DARK or BLACK
      pix(s, dx, dy, c)
      if i % 2 == 0 then pix(s, dx + 1, dy, c) end
    end
  elseif room == 4 then
    -- Lab: clean grid lines (clinical)
    local gc = has_light and DARK or BLACK
    for x = 24, W - 24, 12 do
      for y = 40, 80 do
        pix(s, x, y, gc)
      end
  ...
  elseif room == 6 then
    -- Exit hall: dramatic vertical lines (imposing)
    local gc = has_light and DARK or BLACK
    for x = 22, W - 22, 6 do
      for y = 40, 80, 2 do
        pix(s, x, y, gc)
  ...
  elseif room == 7 then
    -- Garden: vine tendrils on walls
    local vc = has_light and DARK or BLACK
    for i = 0, 5 do
      local bx = 28 + i * 20
      local by = 42
      for j = 0, 8 do
        local ox = math.floor(math.sin(j * 0.8 + i) * 3)
        pix(s, bx + ox, by + j * 4, vc)
        pix(s, bx + ox + 1, by + j * 4 + 1, vc)
      end
    end
    -- Moonlight beam from above (garden only)
    if has_light then
      for y = 0, 38 do
        local bw = math.floor(y / 38.0 * 12) + 2
        local bx = 80 - bw / 2
        for dx = 0, bw - 1 do
          local xx = math.floor(bx) + dx
          if xx >= 0 and xx < W then
            local cur = pix(s, xx, y)
            if cur and cur < WHITE then
              pix(s, xx, y, cur + 1)
            end
          end
        end
...
local function draw_room_base(s)
  -- Ceiling with room-specific dithering
  for y = 0, 39 do
    local depth = y / 40.0
    local shade = has_light and (depth < 0.3 and 2 or 1) or (depth < 0.3 and 1 or 0)
    -- Apply flicker
    if flicker_val ~= 0 and has_light then
      shade = clamp(shade + flicker_val, 0, 3)
    end
    for x = 0, W - 1 do
      local dith = room_dither_test(cur_room, x, y)
      local c = dith and clamp(shade - 1, 0, 3) or shade
      pix(s, x, y, c)
    end
  end

  -- Floor with room-specific dithering
  for y = 80, H - 11 do
  ...
  -- Back wall with slight room shade variation
  local wall_c = has_light and LIGHT or DARK
  if flicker_val ~= 0 and has_light then
    wall_c = clamp(wall_c + flicker_val, 0, 3)
  end
  rectf(s, 0, 40, W, 40, wall_c)
  ...
  -- Apply room-specific textures
  draw_room_texture(s, cur_room)
end
...
local function draw_terminal(s, x, y, powered)
  ...
  if powered then
    ...
    -- Enhanced terminal cursor with flicker
    local t = frame() % 40
    if t < 25 then
      rectf(s, x + 4, y + 12, 4, 2, LIGHT)
    elseif t < 30 then
      rectf(s, x + 4, y + 12, 4, 2, DARK)
    end
    text(s, ">_", x + 4, y + 4, LIGHT)
    -- Scanline effect on terminal screen
    local scan_y = (frame() % 14)
    if scan_y < 14 then
      local sy = y + 2 + scan_y
      if sy < y + 16 then
        for sx = x + 2, x + 25 do
          local cur = pix(s, sx, sy)
          if cur and cur < WHITE then
            pix(s, sx, sy, cur + 1)
          end
        end
...
local function draw_painting(s, x, y, w, h)
  ...
  -- Eyes that follow (subtle flicker)
  if has_light and frame() % 90 > 5 then
    local ex_off = math.floor(math.sin(frame() * 0.02) * 1)
    pix(s, x + w // 2 - 2 + ex_off, y + h // 3 - 1, WHITE)
    pix(s, x + w // 2 + 2 + ex_off, y + h // 3 - 1, WHITE)
  end
end
...
local function draw_chair(s, x, y)
  ...
  -- Headpiece
  circ(s, x + 20, y + 4, 6, c)
  -- Wires dangling with subtle sway
  for i = 0, 2 do
    local wx = x + 16 + i * 4
    local sway = math.floor(math.sin(frame() * 0.08 + i * 1.5) * 1.5)
    line(s, wx, y - 4, wx + sway, y, c)
  end
  -- Electrode sparks (occasional)
  if has_light and frame() % 45 < 3 then
    local sp = x + 16 + math.random(8)
    pix(s, sp, y - 3, WHITE)
    pix(s, sp + 1, y - 4, LIGHT)
  end
end
...
local function draw_tape_recorder(s, x, y)
  ...
  if (t + 5) % 20 < 10 then
    pix(s, x + 14, y + 6, has_light and LIGHT or DARK)
  end
end
    ...
    {x=50, y=40, w=40, h=38, name="exit_blast_door",
      draw=function(s, hs)
        ...
        if flags.escaped then
          -- Bright white light pouring through
          rectf(s, hs.x + 2, hs.y + 2, hs.w - 4, hs.h - 4, WHITE)
          -- Light spill effect
          for dy = -3, 3 do
            for dx = -2, 2 do
              local px = hs.x - 2 + dx
              local py = hs.y + hs.h / 2 + dy
              if px >= 0 and px < W then
                pix(s, px, math.floor(py), LIGHT)
              end
              px = hs.x + hs.w + 1 + dx
              if px >= 0 and px < W then
                pix(s, px, math.floor(py), LIGHT)
    ...
    {x=40, y=44, w=50, h=28, name="garden_plants",
      draw=function(s, hs)
        ...
        for i = 0, 4 do
          ...
          -- Organic curve for leaves
          local sway = math.floor(math.sin(frame() * 0.03 + i * 1.2) * 1.5)
          line(s, px, hs.y + 18, px + 3 + sway, hs.y + 16, c)
          line(s, px, hs.y + 20, px - 2 + sway, hs.y + 17, c)
        end
        -- Green shoot (life persists)
        if has_light then
          local shoot_x = hs.x + 26
          local shoot_sway = math.floor(math.sin(frame() * 0.05) * 0.5)
          pix(s, shoot_x + shoot_sway, hs.y + 22, LIGHT)
          pix(s, shoot_x + shoot_sway, hs.y + 23, LIGHT)
          pix(s, shoot_x + 1 + shoot_sway, hs.y + 21, WHITE)
        end
      end,
      interact=function()
  ...
  hotspots[74] = {
    {x=45, y=42, w=40, h=30, name="garden_window",
      draw=function(s, hs)
        ...
        local stars = {
          {6, 6}, {14, 10}, {10, 4}, {22, 7},
          {8, 14}, {18, 5}, {26, 12}, {30, 8}
        }
        for si, st in ipairs(stars) do
          local twinkle = math.sin(frame() * 0.12 + si * 1.7)
          local sc = twinkle > 0.3 and WHITE or (twinkle > -0.2 and LIGHT or DARK)
          pix(s, hs.x + st[1], hs.y + st[2], sc)
        end
      end,
      interact=function()
...
local function draw_game(s)
  ...
  -- Draw particles (before HUD, after scene)
  draw_particles(s)
  ...
  -- Enhanced crosshair cursor with smooth blink
  local cf = frame() % 30
  local cc
  if cf < 12 then cc = WHITE
  elseif cf < 18 then cc = LIGHT
  elseif cf < 24 then cc = DARK
  else cc = LIGHT end
  ...
  -- Center dot
  pix(s, cur_x, cur_y, cf < 15 and WHITE or LIGHT)
  ...
  -- Typewriter overlay with enhanced cursor
  if tw_text then
    -- Slightly dithered background for typewriter
    for dy = 0, H - 23 do
      for dx = 0, W - 13 do
        local x, y = 6 + dx, 6 + dy
        if (x + y) % 3 == 0 then
    ...
    for i = vis_start, vis_end do
      ...
      -- Enhanced 3-phase blinking cursor
      if not tw_done and line_start + #visible_ln >= chars_shown - 1 and #visible_ln < #ln then
        local blink = frame() % 12
        local cx = 10 + #visible_ln * 4
        if blink < 5 then
          -- Solid block
          rectf(s, cx, y_pos, 3, 7, WHITE)
        elseif blink < 8 then
          -- Half cursor (underscore)
          rectf(s, cx, y_pos + 5, 3, 2, LIGHT)
        end
  ...
  if trans_timer > 0 then
    ...
    if trans_phase == "out" then
      -- Closing: iris shrinks from full to zero
      t = trans_timer / 20.0
    else
      -- Opening: iris grows from zero to full
      t = 1.0 - (trans_timer / 20.0)
    end
    -- Iris radius based on t (0=closed, 1=open)
    local max_r = math.sqrt(W * W + H * H) / 2 + 10
    local r = t * max_r
    local icx, icy = cur_x, cur_y
    -- Black out pixels outside the iris circle
    for y = 0, H - 1 do
      for x = 0, W - 1 do
        local dx = x - icx
        local dy = y - icy
  ...
  -- Heartbeat screen pulse overlay
  if heartbeat_active and heartbeat_timer > 0 then
    local pulse = heartbeat_timer % 15
    if pulse < 3 then
      -- Brief flash: brighten center slightly
      local cx, cy = W / 2, H / 2
      for dy = -8, 8 do
        for dx = -12, 12 do
          local px, py = math.floor(cx + dx), math.floor(cy + dy)
          if px >= 0 and px < W and py >= 0 and py < H - 10 then
            local cur = pix(s, px, py)
            if cur and cur < WHITE then
              pix(s, px, py, cur + 1)
            end
          end
...
local function draw_title(s)
  ...
  -- Atmospheric dithered border
  for y = 0, H - 1 do
    for x = 0, W - 1 do
      local ex = math.min(x, W - 1 - x)
      local ey = math.min(y, H - 1 - y)
      if ex < 4 or ey < 4 then
        if (x + y) % 3 == 0 then
          pix(s, x, y, DARK)
        end
      end
  ...
  -- Flickering title with subtle phase
  local f = frame()
  local flicker = f % 120
  if flicker < 100 or (flicker > 105 and flicker < 108) then
  ...
  if title_tw_pos < #title_text then
    local blink = f % 12
    if blink < 6 then
      local last_line = tw_lines_title[#tw_lines_title] or ""
      local cx = 80 + #last_line * 2
      local cy = 42 + (#tw_lines_title - 1) * 8
      rectf(s, cx, cy, 3, 7, LIGHT)
    end
  end
  ...
  if title_tw_done and f % 40 < 28 then
  ...
  -- Enhanced atmosphere particles (more, varied)
  for i = 1, 10 do
    local px = (f * 7 + i * 37) % (W - 8) + 4
    local py = (f * 3 + i * 53) % (H - 8) + 4
    local pc = (i % 3 == 0) and LIGHT or DARK
    -- Fade based on position
    if py < 12 or py > H - 12 then pc = DARK end
    pix(s, px, py, pc)
  end
end
...
local function draw_ending(s)
  ...
  -- Light grows from center based on ending (enhanced with dither)
  if ending == 3 then
    local lw = math.min(ending_timer, 120) / 120.0 * 80
    local lx = math.floor(80 - lw / 2)
    local lw_i = math.floor(lw)
    for y = 0, H - 1 do
      for x = lx, lx + lw_i - 1 do
        if x >= 0 and x < W then
          local dist = math.abs(x - 80) / (lw / 2 + 1)
          if dist < 0.5 then
            pix(s, x, y, LIGHT)
          elseif (x + y) % 2 == 0 then
            pix(s, x, y, DARK)
  ...
  for i = 1 + scroll, math.min(#lines, scroll + max_vis) do
    ...
    -- Cursor on current line during typing
    if ending_tw_pos < #txt and line_prev + #visible >= chars_shown - 1 and #visible < #ln then
      local blink = frame() % 12
      if blink < 5 then
        local cx = 12 + #visible * 4
        local cy = 6 + (i - 1 - scroll) * 9
        rectf(s, cx, cy, 3, 7, WHITE)
      end
    end
  end

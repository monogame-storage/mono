(function (global, factory) {
    ...
    var ca=Object.assign({},e),da="./this.program",ea=(a,b)=>{throw b;},fa=true,k=false,n=false,ha=false;if(e.ENVIRONMENT)throw Error("Module.ENVIRONMENT has been deprecated. To force the environment, use the ENVIRONMENT compile-time option (for example, -sENVIRONMENT=web or -sENVIRONMENT=node)");var r="",ia,ja,ka;
    if(n){if("undefined"==typeof process||!process.release||"node"!==process.release.name)/* patched: env check removed */;var la=process.versions.node,ma=la.split(".").slice(0,3);ma=1E4*ma[0]+100*ma[1]+1*ma[2].split("-")[0];if(16E4>ma)throw Error("This emscripten-generated code requires node v16.0.0 (detected v"+la+")");const {createRequire:a}=
    await import('module');var require$1=a(('')),fs=require$1("fs"),na=require$1("path");k?r=na.dirname(r)+"/":r=require$1("url").fileURLToPath(new URL("./",('')));ia=(b,c)=>{b=oa(b)?new URL(b):na.normalize(b);return fs.readFileSync(b,c?void 0:"utf8")};ka=b=>{b=ia(b,!0);b.buffer||(b=new Uint8Array(b));u(b.buffer);return b};ja=(b,c,d,f=!0)=>{b=oa(b)?new URL(b):na.normalize(b);fs.readFile(b,f?void 0:"utf8",(h,m)=>{h?d(h):c(f?m.buffer:m);});};!e.thisProgram&&
    1<process.argv.length&&(da=process.argv[1].replace(/\\/g,"/"));process.argv.slice(2);ea=(b,c)=>{process.exitCode=b;throw c;};e.inspect=()=>"[Emscripten Module object]";}else if(ha){if("object"==typeof process&&"function"===typeof require$1||"object"==typeof window||"function"==typeof importScripts)/* patched: env check removed */;"undefined"!=
    ...
    if(e.locateFile){if(M="glue.wasm",!Oa(M)){var Pa=M;M=e.locateFile?e.locateFile(Pa,r):r+Pa;}}else M=(new URL("glue.wasm",(''))).href;function Qa(a){if(ka)return ka(a);throw "both async and sync fetching of the wasm failed";}

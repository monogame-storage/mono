...
function drawInputMonitor()
  ...
  text(scr, "U", x + 10, y, ul)
  text(scr, "L", x, y + 8, ll)
  text(scr, "R", x + 20, y + 8, rl)
  text(scr, "D", x + 10, y + 16, dl)
  local al = btn("a") and 3 or 1
  local bl = btn("b") and 3 or 1
  local sl = btn("start") and 3 or 1
  local sel = btn("select") and 3 or 1
  text(scr, "A", x + 50, y + 4, al)
  text(scr, "B", x + 60, y + 4, bl)
  text(scr, "ST", x + 72, y, sl)
  text(scr, "SE", x + 72, y + 10, sel)
end
...
local function shooterDraw()
  ...
  if shooterLives <= 0 then
    ...
    text(scr, "GAME OVER", 120, 80, 3)
    text(scr, "SCORE:" .. shooterScore, 120, 110, 2)
    if flr(frame() / 20) % 2 == 0 then
      text(scr, "PRESS A TO RETRY", 95, 150, 3)
    end
    text(scr, "[B] BACK TO MENU", 95, 180, 1)
    drawInputMonitor()
    return
  end
  ...
  text(scr, "SCORE:" .. shooterScore, 4, 4, 3)
  text(scr, "LV:" .. shooterLevel, 140, 4, 2)
  text(scr, "B:" .. ecount("bullet") .. " E:" .. ecount("enemy") .. " P:" .. ecount("particle"), 4, 14, 2)

  for i = 1, shooterLives do
    sprT(shipId, W - 20 * i, 1)
  end

  text(scr, "[B] MENU", 4, H - 10, 1)
  drawInputMonitor()
end
...
local function cameraDraw()
  ...
  for gx = 0, CAM_MAP_W, 32 do
    line(scr, gx, 0, gx, CAM_MAP_H, 1)
  end
  -- Horizontal lines every 32px
  for gy = 0, CAM_MAP_H, 32 do
    line(scr, 0, gy, CAM_MAP_W, gy, 1)
  end
  ...
  rect(scr, 0, 0, CAM_MAP_W, CAM_MAP_H, 3)

  -- Draw cross at center of map
  local mcx = CAM_MAP_W / 2
  local mcy = CAM_MAP_H / 2
  line(scr, mcx - 20, mcy, mcx + 20, mcy, 2)
  line(scr, mcx, mcy - 20, mcx, mcy + 20, 2)

  -- Draw corner labels (world-space coordinates rendered via spr-affected draw)
  rectf(scr, 4, 4, 40, 12, 0)
  rectf(scr, CAM_MAP_W - 60, 4, 60, 12, 0)
  rectf(scr, 4, CAM_MAP_H - 16, 40, 12, 0)
  rectf(scr, CAM_MAP_W - 60, CAM_MAP_H - 16, 60, 12, 0)
  ...
  circ(scr, flr(camPX), flr(camPY), 12, 3)

  -- HUD (text is NOT affected by cam, so it draws in screen space)
  text(scr, "CAMERA TEST", 4, 4, 3)
  text(scr, "POS:" .. flr(camPX) .. "," .. flr(camPY), 4, 14, 2)
  text(scr, "MAP:" .. CAM_MAP_W .. "x" .. CAM_MAP_H, 4, 24, 1)
  text(scr, "[A] SHAKE  [B] DASH  [START] MENU", 4, H - 10, 1)
  text(scr, "ARROWS: MOVE", 4, H - 20, 1)
  drawInputMonitor()
end
...
local function spritesDraw()
  cls(scr, 0)
  text(scr, "SPRITE GALLERY", 100, 4, 3)
  ...
  for idx = 1, #sprNames do
    ...
    -- Cell background
    if selected then
      rectf(scr, tx, stripY, thumbSize - 2, thumbSize - 2, 1)
      rect(scr, tx - 1, stripY - 1, thumbSize, thumbSize, 3)
    else
      rect(scr, tx, stripY, thumbSize - 2, thumbSize - 2, 1)
    end
  ...
  -- Normal
  text(scr, "NORMAL", leftX, 58, 2)
  sprT(selId, leftX + 2, 68)

  -- Flip-X
  text(scr, "FLIP-X", leftX, 92, 2)
  sprT(selId, leftX + 2, 102, true, false)

  -- Flip-Y
  text(scr, "FLIP-Y", leftX, 126, 2)
  sprT(selId, leftX + 2, 136, false, true)

  -- Both
  text(scr, "BOTH", leftX, 160, 2)
  ...
  text(scr, flipLabel, pvCX - 20, pvCY + 46, 2)

  -- Controls
  text(scr, "LR:SEL A:FLIPX B:FLIPY UD:ROT", 20, H - 20, 1)
  text(scr, "[START] MENU", 4, H - 10, 1)
  drawInputMonitor()
end
...
local function inputDraw()
  cls(scr, 0)
  text(scr, "INPUT MONITOR", 105, 8, 3)
  ...
  -- Up
  local upOn = btn("up")
  circ(scr, dpadX, dpadY - 40, btnR, upOn and 3 or 1)
  if upOn then circf(scr, dpadX, dpadY - 40, btnR - 2, 3) end
  text(scr, "UP", dpadX - 6, dpadY - 44, upOn and 0 or 2)

  -- Down
  local downOn = btn("down")
  circ(scr, dpadX, dpadY + 40, btnR, downOn and 3 or 1)
  if downOn then circf(scr, dpadX, dpadY + 40, btnR - 2, 3) end
  text(scr, "DN", dpadX - 6, dpadY + 36, downOn and 0 or 2)

  -- Left
  local leftOn = btn("left")
  circ(scr, dpadX - 40, dpadY, btnR, leftOn and 3 or 1)
  if leftOn then circf(scr, dpadX - 40, dpadY, btnR - 2, 3) end
  text(scr, "LT", dpadX - 46, dpadY - 4, leftOn and 0 or 2)

  -- Right
  local rightOn = btn("right")
  circ(scr, dpadX + 40, dpadY, btnR, rightOn and 3 or 1)
  if rightOn then circf(scr, dpadX + 40, dpadY, btnR - 2, 3) end
  text(scr, "RT", dpadX + 34, dpadY - 4, rightOn and 0 or 2)
  ...
  -- A button
  local aOn = btn("a")
  circ(scr, actX, actY, btnR, aOn and 3 or 1)
  if aOn then circf(scr, actX, actY, btnR - 2, 3) end
  text(scr, "A", actX - 3, actY - 4, aOn and 0 or 2)

  -- B button
  local bOn = btn("b")
  circ(scr, actX - 36, actY + 10, btnR, bOn and 3 or 1)
  if bOn then circf(scr, actX - 36, actY + 10, btnR - 2, 3) end
  text(scr, "B", actX - 39, actY + 6, bOn and 0 or 2)

  -- Start
  local stOn = btn("start")
  circ(scr, actX - 10, actY + 50, 12, stOn and 3 or 1)
  if stOn then circf(scr, actX - 10, actY + 50, 10, 3) end
  text(scr, "ST", actX - 17, actY + 46, stOn and 0 or 2)

  -- Select
  local seOn = btn("select")
  circ(scr, actX - 40, actY + 50, 12, seOn and 3 or 1)
  if seOn then circf(scr, actX - 40, actY + 50, 10, 3) end
  text(scr, "SE", actX - 47, actY + 46, seOn and 0 or 2)

  -- Labels
  text(scr, "D-PAD", dpadX - 12, dpadY + 65, 2)
  text(scr, "BUTTONS", actX - 40, actY + 75, 2)

  -- Note: B returns to menu, shown at bottom
  text(scr, "(B exits to menu after release)", 50, H - 20, 1)
  text(scr, "[B] MENU", 4, H - 10, 1)
  drawInputMonitor()
end
...
local function soundDraw()
  cls(scr, 0)
  text(scr, "SOUND TEST", 115, 8, 3)

  -- Note display area
  local cx = W / 2
  local cy = 80

  text(scr, "PRESS DIRECTION KEYS TO PLAY NOTES:", 30, 40, 2)

  -- Show note mapping
  text(scr, "UP    = C4", 100, 60, 1)
  text(scr, "DOWN  = E4", 100, 72, 1)
  text(scr, "LEFT  = G4", 100, 84, 1)
  text(scr, "RIGHT = A4", 100, 96, 1)

  -- Current note display
  if soundLastNote ~= "" then
    text(scr, "LAST NOTE: " .. soundLastNote, 100, 120, 3)
    -- Visual indicator
    circf(scr, cx, 150, 20, 3)
    text(scr, soundLastNote, cx - 8, 146, 0)
  else
    text(scr, "LAST NOTE: ---", 100, 120, 1)
    circ(scr, cx, 150, 20, 1)
  end

  -- BGM status
  local bgmLabel = soundBgmOn and "BGM: ON" or "BGM: OFF"
  local bgmColor = soundBgmOn and 3 or 1
  text(scr, bgmLabel, 130, 185, bgmColor)
  text(scr, "[A] TOGGLE BGM", 105, 200, 2)
...
function title_draw()
  ...
  text(scr, "ENGINE TEST SUITE", 85, 25, 3)
  ...
  rectf(scr, bx, 130, 30, 12, 1)
  circf(scr, 280 - flr(math.sin(titleBlink * 0.05) * 20), 136, 6, 2)
  ...
  if flr(titleBlink / 20) % 2 == 0 then
    text(scr, "PRESS START", 115, 180, 3)
  end

  text(scr, "1:HITBOX/2:SPRITE/3:FILL", 60, 200, 1)
  text(scr, "5 TEST MODES INSIDE", 85, 212, 1)
  text(scr, "SHOOTER CAMERA SPRITES INPUT SFX", 25, 224, 1)
...
function play_draw()
  if currentMode == MODE_MENU then
    ...
    text(scr, "SELECT TEST", 115, 20, 3)
    line(scr, 100, 32, 220, 32, 2)
    ...
    for i = 1, #menuItems do
      ...
      if selected then
        -- Highlight bar
        rectf(scr, 30, y - 2, 260, 18, 1)
        -- Cursor arrow
        text(scr, ">", 34, y, 3)
      end

      text(scr, tostring(i) .. ". " .. menuItems[i], 48, y, col)
      text(scr, descriptions[i], 56, y + 9, selected and 2 or 1)
    end

    -- Footer
    line(scr, 30, menuStartY + #menuItems * menuSpacing + 4, 290, menuStartY + #menuItems * menuSpacing + 4, 1)
    text(scr, "UP/DOWN: SELECT   A/START: ENTER", 40, menuStartY + #menuItems * menuSpacing + 12, 2)
...
function gameover_draw()
  ...
  text(scr, "GAME OVER", 120, 80, 3)
  if flr(frame() / 20) % 2 == 0 then
    text(scr, "PRESS A TO CONTINUE", 85, 140, 3)
  end
  drawInputMonitor()
end

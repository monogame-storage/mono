...
```bash
node mono-runner.js main.lua --frames 30
...
```
1. main.lua 수정
2. node mono-runner.js main.lua --frames 5
...
```
$ node mono-runner.js main.lua --frames 1
...
```bash
node mono-runner.js main.lua --frames 100 --input "5:right,10:a,20:up" --record session.replay
...
```bash
node mono-runner.js main.lua --frames 100 --replay session.replay --snapshot actual.txt
...
```bash
# 1. 기대 출력 저장
node mono-runner.js main.lua --frames 30 --snapshot expected.txt

# 2. 엔진 수정 후 동일 상태 검증
node mono-runner.js main.lua --frames 30 --diff expected.txt
```
...
```bash
node mono-runner.js main.lua --frames 120 --determinism 5 --seed 42
```
...
```bash
node mono-runner.js main.lua --frames 30 --coverage --quiet
```
...
```bash
node mono-runner.js --scan ./demo --frames 30 --coverage --quiet
```
...
```bash
node mono-runner.js main.lua --frames 100 --input "5:right,20:a" --trace session.jsonl
```
...
```bash
node mono-runner.js main.lua --frames 120 --golden game.golden --golden-update --seed 42
...
```bash
node mono-runner.js main.lua --frames 120 --golden game.golden --seed 42
# → 기록된 해시와 비교
```
...
```bash
node mono-runner.js main.lua --frames 300 --bench --quiet
```
...
```bash
node mono-runner.js main.lua --frames 60 --fuzz 100
```
...
```bash
node mono-runner.js --scan ./demo --frames 30
```
...
- `.mono/CONTEXT.md`에 mono-runner 사용법 자동 포함

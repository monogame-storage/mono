...
```bash
# 내장 봇 (기본 추적 AI)
node runtime/mono-test.js game.lua --frames 10000 --until "WINNER" --bot --quiet

# 외부 봇 스크립트
node runtime/mono-test.js game.lua --frames 10000 --until "WINNER" --bot bot.lua --quiet

# 10판 AI vs Bot 밸런스 테스트
node runtime/mono-test.js game.lua --frames 20000 --until "WINNER" --bot --runs 10 --quiet
```

...
```bash
# 파일 실행
node runtime/mono-test.js main.lua --frames 10 --vdump

# 인라인 코드 실행 (파일 없이)
node runtime/mono-test.js --source 'cls(0) rectf(10,10,20,20,1) text("HI",50,50,1)' --vdump
...
2. node runtime/mono-test.js main.lua --frames 5 --vdump (Bash tool)
...
```bash
# 도형 그리기 테스트
node runtime/mono-test.js --source 'cls(0) circ(80,72,30,1)' --vdump

# 특정 픽셀 검증
node runtime/mono-test.js --source 'cls(0) pix(80,72,1) print(gpix(80,72))' --console --quiet

# 텍스트 렌더링 확인 (좁은 영역만)
node runtime/mono-test.js --source 'cls(0) text("HELLO WORLD",20,68,1)' --vdump --region 18,66,80,10
```
...
node runtime/mono-test.js main.lua --frames 10 --input "3:a,5:right" --vdump
```
...
```bash
# 기준 스냅샷 생성
node runtime/mono-test.js main.lua --frames 5 --snapshot expected.txt

# 수정 후 비교
node runtime/mono-test.js main.lua --frames 5 --diff expected.txt
...
node runtime/mono-test.js main.lua --frames 10000 --until "WINNER" --quiet --console
# → --until "WINNER" matched at frame 297 (9.9s game time)

# 밸런스 테스트: 10판 돌려서 승률 확인
node runtime/mono-test.js main.lua --frames 10000 --until "WINNER" --quiet --runs 10
# → Results:
#   "WINNER: CPU 9-0": 7/10 (70%)
#   "WINNER: P2 9-5": 3/10 (30%)

# 재현 가능한 테스트: 특정 시드로 고정
node runtime/mono-test.js main.lua --frames 1000 --seed 42 --console --quiet

# 스트레스 테스트: 10000프레임 에러 없이 완주하는지
node runtime/mono-test.js main.lua --frames 10000 --quiet
```
...
```bash
# 내장 봇 (기본 추적 AI)
node runtime/mono-test.js main.lua --frames 10000 --until "WINNER" --bot --quiet

# 외부 봇 스크립트
node runtime/mono-test.js main.lua --frames 10000 --until "WINNER" --bot bot.lua --quiet

# 10판 AI vs Bot 밸런스 테스트
node runtime/mono-test.js main.lua --frames 20000 --until "WINNER" --bot --runs 10 --quiet
```
...
1. **빠른 검증**: `mono-test.js --vdump` (1초 이내, 필요하면 `--region`으로 좁힘)
2. **사람 확인**: `mono-test.js --png` → `Read` 도구로 이미지 열기

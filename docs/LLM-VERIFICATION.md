...
```bash
# 파일 실행
node runtime/mono-test.js main.lua --frames 10 --ascii
...
npm run verify -- main.lua --frames 5 --ascii
```
...
2. node runtime/mono-test.js main.lua --frames 5 --ascii (Bash tool)
3. ASCII 아트로 화면 확인
4. 문제 있으면 수정 반복
```
...
node runtime/mono-test.js main.lua --frames 10 --input "3:a,5:right" --ascii
```
